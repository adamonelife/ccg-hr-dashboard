...
// address (new) and profile_setup_completed_at (new — NULL until someone
// finishes first-login setup, see the PATCH handler below) — self-healed
// the same way lib/documents.mjs and lib/skills.mjs do it, rather than
// requiring a separate migration visit.
...
export async function handleEmployees(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    // unchanged direct-write behaviour.
    try {
