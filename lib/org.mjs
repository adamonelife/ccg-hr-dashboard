...
// (from the `org_units` table, so structure exists even for teams with no
// one staffed yet), and flat reporting lines (from each employee's
// manager_id). Logic is unchanged from the Sheets version — only the data
// source underneath loadOrgUnits() changed.
...
export async function loadOrgUnits() {
  const sql = getSql();
  return sql`SELECT unit_type, unit_name, parent_unit_name FROM org_units ORDER BY unit_name`;
}
...
// Flat department/team name lists, sourced live from org_units — powers the
