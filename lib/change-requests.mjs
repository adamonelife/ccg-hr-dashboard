...
// and only takes effect once approved.
//
// Two distinct roles sets are involved here, deliberately different:
//   - Who's exempt from ever needing to submit a request for their own
//     record: Administrator/HR/Director (FULL_VISIBILITY_ROLES in
//     permissions.mjs) — editing your own record when you're already one
//     of the roles that would approve it is a distinction without a
//     difference. Finance stays out of this — Finance's own edits still
//     go through approval.
//   - Who can review/approve OTHER people's requests (the queue below):
//     STAFF_MANAGEMENT_ROLES (Administrator/Director/HR/Finance) — Finance
//     IS trusted to review others' requests even though their own still
//     need sign-off, same asymmetry Leave approvals already have (you can
//     approve others' but never your own). No extra target-role filtering
//     needed here for the Director/Administrator exclusion that applies
//     elsewhere — Director/Administrator never generate requests in the
//     first place (they're exempt), so the queue can never contain one.
//
...
import { STAFF_MANAGEMENT_ROLES } from './permissions.mjs';
...
export async function handleChangeRequests(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // The review queue — every Administrator/Director/HR/Finance sees the
    // same list (no org-chart scoping here, unlike Leave's approvals;
    // sign-off authority for self-service changes isn't split by team).
    if (scope === 'queue') {
      if (!STAFF_MANAGEMENT_ROLES.has(req.session?.role)) {
    ...
    if (!isSelf && !STAFF_MANAGEMENT_ROLES.has(req.session?.role)) {
      res.status(403).json({ error: 'Insufficient permissions' });
      return;
    }
    const rows = await sql`
