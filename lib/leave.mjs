...
//
// Visibility: same tiered scoping as everything else in Phase 3 (see
// lib/permissions.mjs) — a lead sees and can approve/reject requests for
// anyone in their scope; Administrator/Director see everyone, no
// exceptions; HR/Finance see everyone except a Director's or
// Administrator's own; everyone sees their own regardless.
...
// Both widened to include Finance alongside HR (Adam's "full access to
// leave" for HR + Finance) — actual reach is still bounded by canView
// below (a Director/Administrator target 403s for HR/Finance same as
// everywhere else), so this is just "who's in the broad-access tier at
// all," not "who can act on anyone."
const BALANCE_MANAGE_ROLES = new Set(['Administrator', 'Director', 'HR', 'Finance']);
const SUBMIT_ON_BEHALF_ROLES = new Set(['Administrator', 'Director', 'HR', 'Finance']);
...
export async function handleLeaveBalances(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // Role check plus a canView check on the specific target — the role
    // check alone would let HR/Finance set a Director's/Administrator's
    // allocation, which the visibility rule elsewhere in this app
    // deliberately blocks.
    if (!BALANCE_MANAGE_ROLES.has(req.session?.role) || !(await canView(req.session, body.employee_id))) {
      res.status(403).json({ error: 'Insufficient permissions' });
      return;
    }
...
export async function handleLeaveRequests(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (scope === 'approvals') {
      ...
      if (visible === null) {
        rows = await sql`
          SELECT lr.*, e.full_name, e.nickname
          FROM leave_requests lr
          JOIN employees e ON e.employee_id = lr.employee_id
          WHERE lr.status = 'Pending' AND lr.employee_id != ${req.session?.employee_id || ''}
        ...
        rows = await sql`
          ...
          FROM leave_requests lr
          JOIN employees e ON e.employee_id = lr.employee_id
          WHERE lr.status = 'Pending' AND lr.employee_id = ANY(${visible})
          ORDER BY lr.requested_at
        `;
      }
  ...
  if (req.method === 'POST') {
    const body = req.body || {};
    if (!body.employee_id || !body.leave_type || !body.start_date || !body.end_date || !body.reason?.trim()) {
      res.status(400).json({ error: 'employee_id, leave_type, start_date, end_date and reason are required' });
      return;
    }
    if (!LEAVE_TYPES.includes(body.leave_type)) {
      res.status(400).json({ error: `leave_type must be one of: ${LEAVE_TYPES.join(', ')}` });
    ...
    // Same reasoning as the balance-management check above — the role set
    // says who's eligible to submit on someone else's behalf at all, but
    // canView still applies per-target, so HR/Finance can't file a leave
    // request for a Director/Administrator.
    const canActOnBehalf = SUBMIT_ON_BEHALF_ROLES.has(req.session?.role) && (await canView(req.session, body.employee_id));
    if (!isSelf && !canActOnBehalf) {
      res.status(403).json({
        error: 'Only the employee themselves, HR, Finance, Director or Administrator can submit a request for this person',
      });
      return;
    }
