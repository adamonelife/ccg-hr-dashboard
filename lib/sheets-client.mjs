...
//
// NO LONGER THE LIVE DATABASE — Postgres (see lib/db.mjs, db/schema.sql) is
// now the app's data store. This file is kept for two things only:
//   1. db/migrate-from-sheets.mjs — the one-time script that copied the old
//      Sheet into Postgres. Needed if you ever re-run that script.
//   2. A starting point for Drive integration in Phase 2 (Documents) — that
//      will need its own JWT/token logic with a different OAuth scope
//      (drive, not spreadsheets), likely a sibling file rather than a reuse
//      of this one as-is.
// Every live route (employees, org-chart, org-units, salary-history,
// promotion-history, skills) reads/writes Postgres now, not this file.
//
