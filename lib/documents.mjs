...
//     that employee themselves; Administrator/Director see everyone's,
//     no exceptions; HR/Finance see everyone's EXCEPT a Director's or
//     Administrator's own (RESTRICTED_TARGET_ROLES, imported from
//     lib/permissions.mjs — same exclusion applied to Employee Card/
//     Skills/Leave/Change-requests). Deliberately NOT the same org-chart/
//     lead visibility used for Employee Cards/Leave — a Team Lead does NOT
//     get their team's personal documents just for being a lead, only
//     these roles do (Adam's explicit spec). Files themselves stay in
//     Google Drive — this table only stores a link + metadata, same
//     pattern as the rest of the app's Drive integration.
//
...
//     Administrator/HR/Director/Finance (not leads) — Finance added per
//     Adam's explicit ask for HR/Finance parity; previously excluded here.
//
...
import { RESTRICTED_TARGET_ROLES } from './permissions.mjs';
...
// is. Deliberately includes Finance, and deliberately flat rather than
// org-chart-scoped (unlike lib/permissions.mjs's tier-1 lead-based
// scoping) — a Team Lead does NOT get their team's personal documents just
// for being a lead, only these four roles do. Administrator/Director are
// fully unrestricted; HR/Finance get everyone EXCEPT RESTRICTED_TARGET_ROLES
// (Director/Administrator — "Operations + Executive"), same exclusion
// lib/permissions.mjs applies everywhere else, imported from there rather
// than redefined here so the two can't drift apart.
const DOC_UNRESTRICTED_ROLES = new Set(['Administrator', 'Director']);
const DOC_BROAD_ACCESS_ROLES = new Set(['HR', 'Finance']);

async function canViewPersonalDocs(sql, session, employeeId) {
  if (!session) return false;
  if (session.employee_id === employeeId) return true;
  if (DOC_UNRESTRICTED_ROLES.has(session.role)) return true;
  if (!DOC_BROAD_ACCESS_ROLES.has(session.role)) return false;
  const [target] = await sql`SELECT permission_role FROM employees WHERE employee_id = ${employeeId}`;
  // Unknown/deleted employee_id: fall through to false rather than true —
  // nothing to legitimately view either way.
  return Boolean(target) && !RESTRICTED_TARGET_ROLES.has(target.permission_role);
}

// Same rule can upload/delete on someone else's behalf; uploading your own
// is always allowed regardless of role.
async function canManagePersonalDocs(sql, session, employeeId) {
  return canViewPersonalDocs(sql, session, employeeId);
}

// Finance added per Adam's explicit ask for HR/Finance parity — was
// Administrator/HR/Director only.
const COMPANY_UPLOAD_ROLES = new Set(['Administrator', 'HR', 'Director', 'Finance']);
...
export async function handlePersonalDocuments(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (!(await canViewPersonalDocs(sql, req.session, employeeId))) {
  ...
  if (req.method === 'POST') {
    ...
    if (!(await canManagePersonalDocs(sql, req.session, fields.employee_id))) {
      res.status(403).json({ error: 'Insufficient permissions' });
      return;
    }
  ...
  if (req.method === 'DELETE') {
    ...
    if (!(await canManagePersonalDocs(sql, req.session, existing.employee_id))) {
      res.status(403).json({ error: 'Insufficient permissions' });
      return;
    }
...
export async function handleCompanyDocuments(req, res) {
  ...
  if (req.method === 'POST') {
    const { fields, file } = await getRequestData(req);
    if (!fields.access_role || !fields.title?.trim()) {
      res.status(400).json({ error: 'access_role and title are required' });
      return;
    }
    ...
    // "Folder" and access_role are the same thing now — one folder per
    // permission tier, matching Adam's original spec exactly ("a subset
    // of folders that will match each access group"). Ignoring any
    // client-supplied folder value rather than trusting it, so this can
    // never drift out of sync with the tier that actually gates it.
    const folder = fields.access_role;
