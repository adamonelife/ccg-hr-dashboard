...
// If a tab was never created (e.g. Skills, if that feature was never
// actually used on Sheets), Google's API fails with "Unable to parse
// range" — treat that specific failure as "nothing to migrate for this
// table" rather than aborting the whole run. Any other error still throws.
async function safeReadRange(sheetId, range) {
  try {
    return await readRange(sheetId, range);
  } catch (err) {
    if (err.message.includes('Unable to parse range')) return [];
    throw err;
  }
}
...
async function migrateOrgUnits(sql, sheetId) {
  const rows = await safeReadRange(sheetId, 'OrgUnits!A2:C');
...
async function migrateEmployees(sql, sheetId) {
  const rows = await safeReadRange(sheetId, 'Employees!A2:AI');
  ...
  for (const e of employees) {
    const record = {
      ...
      // Blank sheet cells become 'NA' rather than null — matches the
      // dropdown's option list (see EmployeeForm.jsx and schema.sql).
      religion: e.religion || 'NA',
...
async function migrateSalaryHistory(sql, sheetId) {
  const rows = await safeReadRange(sheetId, 'SalaryHistory!A2:G');
...
async function migratePromotionHistory(sql, sheetId) {
  const rows = await safeReadRange(sheetId, 'PromotionHistory!A2:G');
  const entries = rowsToObjects(
...
async function migrateSkills(sql, sheetId) {
  const rows = await safeReadRange(sheetId, 'Skills!A2:F');
