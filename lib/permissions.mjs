...
// submitting a request for their own record"). Finance was deliberately
// left out of this when the HR/Finance staff-management tier was first
// built — being trusted to *review* others' requests
...
// — but per Adam's explicit follow-up ("can Finance and HR have the same
// access?"), that asymmetry has been removed: Finance is now exempt too,
// same as HR.
export const FULL_VISIBILITY_ROLES = new Set(['Administrator', 'Director', 'HR', 'Finance']);
