...
// 'Company' deliberately dropped from the choices offered when creating a
// new unit — CC/CC Landscape/Pelago are separate companies externally, but
// organisationally they're just teams under Operations, so new units
// should always be created as Team (or Department/Group). The DB's
// unit_type CHECK constraint still allows 'Company' for any existing rows/
// flexibility later, this is a UI-level choice only.
export const UNIT_TYPES = ['Group', 'Department', 'Team'];

// Shared by handleOrgChart and handleOrgUnits so both read org_units the
// same way. Ordered by sort_order first (NULLs — i.e. every unit that's
// never had a custom order set — sort last, alphabetically), so a manual
// re-order (see handleOrgUnits' PATCH) only affects the units it's
// actually been set on.
export async function loadOrgUnits() {
  const sql = getSql();
  return sql`
    SELECT unit_type, unit_name, parent_unit_name, lead_employee_id, sort_order
    FROM org_units
    ORDER BY sort_order NULLS LAST, unit_name
  `;
}
...
// Director only. HR/Finance can view the org chart (no route-level block
// on GET above) but never mutate structure — matches
// lib/permissions.mjs's UNRESTRICTED_VISIBILITY_ROLES, same two roles
// that see literally everyone with no exceptions.
...
export async function handleOrgUnits(req, res) {
  ...
  // Assign/change/clear a unit's lead, and/or move it under a different
  // parent (re-parent). Renaming a unit's own name is still not supported
  // here — that needs cascading into every employee.department/team string
  // that points at the old name, which isn't built. Reparenting doesn't
  // have that problem: employees reference their own team/department by
  // name, not by its position in the tree, so moving a unit elsewhere in
  // the hierarchy is safe on its own.
  if (req.method === 'PATCH') {
    const body = req.body || {};
    if (!body.unit_name) {
    ...
    const updates = {};
    if (Object.prototype.hasOwnProperty.call(body, 'lead_employee_id')) {
      updates.lead_employee_id = body.lead_employee_id || null;
    }
    if (Object.prototype.hasOwnProperty.call(body, 'parent_unit_name')) {
      const newParent = body.parent_unit_name || null;
      if (newParent === body.unit_name) {
        res.status(400).json({ error: 'A unit cannot be its own parent' });
        return;
      }
      if (newParent) {
        // Block moving a unit under itself or one of its own sub-units —
        // that would create a loop the tree view can't render.
        const descendants = await sql`
          WITH RECURSIVE descendants AS (
            SELECT unit_name FROM org_units WHERE parent_unit_name = ${body.unit_name}
            UNION ALL
            SELECT ou.unit_name FROM org_units ou
            JOIN descendants d ON ou.parent_unit_name = d.unit_name
          )
          SELECT unit_name FROM descendants
        `;
        if (descendants.some((d) => d.unit_name === newParent)) {
          res.status(400).json({ error: 'Cannot move a unit under one of its own sub-units' });
          return;
        }
      }
      updates.parent_unit_name = newParent;
    }
    if (Object.prototype.hasOwnProperty.call(body, 'sort_order')) {
      const order = body.sort_order === '' || body.sort_order === null ? null : Number(body.sort_order);
    ...
    if (Object.keys(updates).length === 0) {
      res.status(400).json({
        error: 'Nothing to update — provide lead_employee_id, parent_unit_name, and/or sort_order',
      });
      return;
    }
    ...
    try {
      const [row] = await sql`
        UPDATE org_units SET ${sql(updates)}
